...
@section('content')
    ...
    <div class="w-1/2 m-auto px-40">
        ...
        <form method="POST" action="/editpost/{{ $post->id }}" class="mt-16  gap-10 flex flex-col w-full">
            @csrf
            @method('PATCH')
            <div class="flex flex-col gap-1">
                <label for="title" class="text-xl font-medium ">Title</label>
                <input name="title" id="title"
                    class="border h-10 border-slate-500 p-2 outline-none rounded-md resize-none"
                    value="{{ $post->title }}"></input>
            </div>
            <div class="flex flex-col gap-1">
                <label for="content" class="text-xl font-medium ">Content</label>
                <textarea name="content" id="content" class="border border-slate-500 h-32 outline-none rounded-md p-2 resize-none">{{ $post->content }}</textarea>
            </div>
            <input type="hidden" name="user_id" value="{{ auth()->user()->id }}">
            <div>
                <button type="submit"
                    class="w-32 mr-5 text-white tracking-wider rounded-md bg-sky-500 py-2 px-7 text-lg font-medium">Save</button>
